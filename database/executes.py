from database.exists import (deputy_exists, party_exists, sitting_exists,
                             vote_exists, voting_exists)
from database.inserts import (insert_deputy, insert_parties, insert_sittings,
                              insert_vote, insert_voting)
...
def execute_votings(glosowania_dataframe):
    ...
    for index, row in glosowania_dataframe.iterrows():
        if voting_exists(cursor, row['nr_glosowania'], row['opis']):
            ...
            insert_voting(cursor, row['id_posiedzenia'], row['nr_glosowania'], row['opis'])
...
def execute_votes(votes_dataframe):
    ...
    for index, row in votes_dataframe.iterrows():
        if vote_exists(cursor, row['id_posel'], row['id_glosowania']):
            ...
            insert_vote(cursor, row['id_partia'], row['id_posel'], row['id_glosowania'], row['glos'], row['data_glosu'])
