...
from html_scraper import get_deputies, get_parties, get_sittings, get_votes, get_votings
...
def parties_gui(choice):
    ...
    if choice == 1:
        ...
        parties_status_label.config(
            text="Status: partie dodane do bazy danych", fg="green"
        )
    end = timer()
...
def deputies_gui(choice):
    ...
    if choice == 1:
        ...
        deputies_status_label.config(
            text="Status: poslowie dodani do bazy danych", fg="green"
        )
    end = timer()
    duration = str(timedelta(seconds=end - start))[:-7]
...
def sittings_gui(choice):
    ...
    if choice == 1:
        ...
        sittings_status_label.config(
            text="Status: posiedzenia dodane do bazy danych", fg="green"
        )
    end = timer()
    duration = str(timedelta(seconds=end - start))[:-7]
...
def votings_gui(choice):
    ...
    if choice == 1:
        ...
        votings_status_label.config(
            text="Status: głosowania dodane do bazy danych", fg="green"
        )
    end = timer()
    duration = str(timedelta(seconds=end - start))[:-7]
...
def votes_gui(choice):
    ...
    if choice == 1:
        votes_dataframe = get_votes(
            id_deputy_from, id_deputy_to, id_sitting_from, id_sitting_to
        )
        votes_status_label.config(text="Status: głosy pobrane", fg="green")
    elif choice == 2:
        execute_votes(votes_dataframe)
        votes_status_label.config(
            text="Status: głosy dodane do bazy danych", fg="green"
        )
...
partie_wyszukaj_button = tk.Button(
    root,
    text="Wyszukaj",
    font=font_button,
    cursor=cursor,
    command=lambda: parties_gui(1),
)
partie_wyszukaj_button.grid(column=1, row=0)

partie_import_button = tk.Button(
    root,
    text="Importuj",
    font=font_button,
    cursor=cursor,
    command=lambda: parties_gui(2),
)
partie_import_button.grid(column=2, row=0)

parties_status_label = tk.Label(
    root, text="Status:", font=font_status_label, bg=bg_colour, fg="black"
)
parties_status_label.grid(column=3, row=0, padx=padx, sticky="w")

parties_time_label = tk.Label(
    root, text="Czas: ", font=font_status_label, bg=bg_colour, fg="black"
)
parties_time_label.grid(column=3, row=0, padx=padx, sticky="ws")

# POSŁOWIE
poslowie_label = tk.Label(
    root, text="Posłowie", font=font_label, bg=bg_colour, fg="black"
)
poslowie_label.grid(column=0, row=1, padx=padx, sticky="w")

poslowie_wyszukaj_button = tk.Button(
    root,
    text="Wyszukaj",
    font=font_button,
    cursor=cursor,
    command=lambda: deputies_gui(1),
)
poslowie_wyszukaj_button.grid(column=1, row=1)

poslowie_import_button = tk.Button(
    root,
    text="Importuj",
    font=font_button,
    cursor=cursor,
    command=lambda: deputies_gui(2),
)
poslowie_import_button.grid(column=2, row=1)

deputies_status_label = tk.Label(
    root, text="Status:", font=font_status_label, bg=bg_colour, fg="black"
)
deputies_status_label.grid(column=3, row=1, padx=padx, sticky="w")

deputies_time_label = tk.Label(
    root, text="Czas: ", font=font_status_label, bg=bg_colour, fg="black"
)
deputies_time_label.grid(column=3, row=1, padx=padx, sticky="ws")

# POSIEDZENIA
posiedzenia_label = tk.Label(
    root, text="Posiedzenia", font=font_label, bg=bg_colour, fg="black"
)
posiedzenia_label.grid(column=0, row=2, padx=padx, sticky="w")

posiedzenia_wyszukaj_button = tk.Button(
    root,
    text="Wyszukaj",
    font=font_button,
    cursor=cursor,
    command=lambda: sittings_gui(1),
)
posiedzenia_wyszukaj_button.grid(column=1, row=2)

posiedzenia_import_button = tk.Button(
    root,
    text="Importuj",
    font=font_button,
    cursor=cursor,
    command=lambda: sittings_gui(2),
)
posiedzenia_import_button.grid(column=2, row=2)

sittings_status_label = tk.Label(
    root, text="Status:", font=font_status_label, bg=bg_colour, fg="black"
)
sittings_status_label.grid(column=3, row=2, padx=padx, sticky="w")

sittings_time_label = tk.Label(
    root, text="Czas: ", font=font_status_label, bg=bg_colour, fg="black"
)
sittings_time_label.grid(column=3, row=2, padx=padx, sticky="ws")

# GŁOSOWANIA
glosowania_label = tk.Label(
    root, text="Głosowania", font=font_label, bg=bg_colour, fg="black"
)
glosowania_label.grid(column=0, row=3, padx=padx, sticky="w")

glosowania_wyszukaj_button = tk.Button(
    root,
    text="Wyszukaj",
    font=font_button,
    cursor=cursor,
    command=lambda: votings_gui(1),
)
glosowania_wyszukaj_button.grid(column=1, row=3)

glosowania_import_button = tk.Button(
    root,
    text="Importuj",
    font=font_button,
    cursor=cursor,
    command=lambda: votings_gui(2),
)
glosowania_import_button.grid(column=2, row=3)

votings_status_label = tk.Label(
    root, text="Status:", font=font_status_label, bg=bg_colour, fg="black"
)
votings_status_label.grid(column=3, row=3, padx=padx, sticky="w")

votings_time_label = tk.Label(
    root, text="Czas: ", font=font_status_label, bg=bg_colour, fg="black"
)
...
glosy_wyszukaj_button = tk.Button(
    root, text="Wyszukaj", font=font_button, cursor=cursor, command=lambda: votes_gui(1)
)
glosy_wyszukaj_button.grid(column=1, row=4)

glosy_import_button = tk.Button(
    root, text="Importuj", font=font_button, cursor=cursor, command=lambda: votes_gui(2)
)
glosy_import_button.grid(column=2, row=4)

votes_status_label = tk.Label(
    root, text="Status:", font=font_status_label, bg=bg_colour, fg="black"
)
votes_status_label.grid(column=3, row=4, padx=padx, sticky="w")

votes_time_label = tk.Label(
    root, text="Czas: ", font=font_status_label, bg=bg_colour, fg="black"
)
votes_time_label.grid(column=3, row=4, padx=padx, sticky="ws")

glosy_id_posla_label_od = tk.Label(
    root, text="ID posła od:", font=font_button, bg=bg_colour, fg="black"
)
...
glosy_id_posla_label_do = tk.Label(
    root, text="ID posła do:", font=font_button, bg=bg_colour, fg="black"
)
...
glosy_id_posiedzenia_label_od = tk.Label(
    root, text="ID posiedzenia od:", font=font_button, bg=bg_colour, fg="black"
)
...
glosy_id_posiedzenia_label_do = tk.Label(
    root, text="ID posiedzenia do:", font=font_button, bg=bg_colour, fg="black"
)
