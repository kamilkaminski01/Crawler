...
from utils.utils import (
    get_id_deputies_list,
    get_id_sittings_list,
    get_last_name,
    get_name,
    get_parties_from_database,
    get_votes_from_database,
    load_dataframe_with_pandas,
    load_site,
)
...
def get_sittings():
    ...
    try:
        ...
        dataframe = dataframe.rename(
            columns={"Nr pos. Sejmu": "nr_posiedzenia", "Data pos. Sejmu": "data"}
        )
...
def get_votings():
    ...
    for id in id_sittings_list:
        ...
        try:
            ...
            dataframe = dataframe.rename(
                columns={"Nr": "nr_glosowania", "Temat": "opis"}
            )
            dataframe.insert(1, "id_posiedzenia", id)
            joined_dataframe = pd.concat(
                [joined_dataframe, pd.DataFrame.from_records(dataframe)]
            )
            url.strip(id)
        except Exception as e:
...
def get_votes(id_deputy_from, id_deputy_to, id_sitting_from, id_sitting_to):
    ...
    parties_database_table = get_parties_from_database()
    ...
    def check_deputies_party_id(url, name):
        ...
        while found is False:
            try:
                    ...
                    if fnmatch.fnmatch(link, url_nr_sitting):
                        ...
                        for party in soup.find_all("a"):
                            ...
                            if fnmatch.fnmatch(party, url_party_voting):
                                ...
                                found_party_name = found_party_name[
                                    found_party_name.index("=") :
                                ].strip("=")
                ...
                for index, row in parties_database_table.iterrows():
    ...
    for i in full_id_sittings_list[
        full_id_sittings_list.index(id_sitting_from) : full_id_sittings_list.index(
            id_sitting_to
        )
        + 1
    ]:
    ...
    for j in full_id_deputies_list[
        full_id_deputies_list.index(id_deputy_from) : full_id_deputies_list.index(
            id_deputy_to
        )
        + 1
    ]:
                ...
                try:
                    ...
                    for link in soup.find_all("a"):
                        ...
                        if fnmatch.fnmatch(link, temp_sitting_link):
                            id_votes_list = []
                            id_posiedzenia = link[link.index("IdDnia=") :].strip(
                                "IdDnia="
                            )
                            ...
                            date = header_tag[
                                header_tag.index("dniu ") : header_tag.index(" na")
                            ].strip("dniu ")
                            year = date[date.index("-") + 4 :]
                            month = date[date.index("-") : date.index("-") + 4].strip(
                                "-"
                            ...
                            id_deputies_party = check_deputies_party_id(
                                temp_link, new_full_name
                            )
                            ...
                            for (
                                index1,
                                row_glosowania,
                            ) in votes_database_table.iterrows():
                                if row_glosowania["id_posiedzenia"] == int(
                                    id_posiedzenia
                                ):
                                    ...
                                    for index2, row_dataframe in dataframe.iterrows():
                                        ...
                                        if (
                                            vote_number == int(vote_number2)
                                            and description == description2
                                        ):
                            ...
                            dataframe["data_glosu"] = dataframe["data_glosu"].fillna(
                                method="ffill"
                            )
